...
export function setupHands({ container, size, camera, sceneCSS, video, cursor }) {
  ...
  function dispatch(type, x, y, button = 0, buttons = 0) {
  ...
  hands.onResults(({ multiHandLandmarks: [lm] }) => {
    ...
    if (!lm) {
      ...
      if (leftPinch) dispatch('pointerup', lastX, lastY, 0, 0);
      if (rightPinch) dispatch('pointerup', lastX, lastY, 2, 0);
    ...
    dispatch('pointermove', lastX, lastY, 0, leftPinch ? 1 : rightPinch ? 2 : 0);
    ...
    if (dist < 40) {
      if (!leftPinch && !rightPinch) {
        leftPinch = true;
        pinchStart = now;
        dispatch('pointerdown', lastX, lastY, 0, 1);
      ...
      leftPinch = false;
      dispatch('pointerup', lastX, lastY, 0, 0);
    ...
    if (distR < 40) {
      if (!rightPinch && !leftPinch) {
        rightPinch = true;
        pinchStart = now;
        dispatch('pointerdown', lastX, lastY, 2, 2);
      }
    } else if (rightPinch) {
      rightPinch = false;
      dispatch('pointerup', lastX, lastY, 2, 0);
